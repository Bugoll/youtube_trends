...
Uses the Anthropic API (claude-haiku-4-5) to:
  1. Classify macro_theme for videos where keyword matching returns "Разное".
  2. Generate a brief summary for videos that have no summary.

Runs incrementally — only processes videos not yet enriched (or enriched
more than REENRICH_AFTER_DAYS ago). Safe to run repeatedly — idempotent.
...
def _classify_theme(client: Any, entry: dict) -> str | None:
...
def _generate_summary(client: Any, entry: dict) -> str | None:
...
def enrich(history: dict[str, Any], limit: int, force: bool) -> int:
    ...
    if not api_key:
        print("ANTHROPIC_API_KEY not set — skipping LLM enrichment.")
        return 0

    try:
        import anthropic
    except ImportError:
        print("anthropic package not installed — skipping LLM enrichment.", file=sys.stderr)
        return 0
    ...
    print(f"Enriching {len(candidates)} video(s) via {MODEL}…")
    updated = 0
    ...
    for entry, needs_theme, needs_summary in candidates:
        ...
        if needs_theme:
            theme = _classify_theme(client, entry)
        ...
        if needs_summary:
            summary = _generate_summary(client, entry)
            if summary:
